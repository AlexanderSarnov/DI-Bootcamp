...
// Declare variable to store an array of gifs
let gifs = [];
...
searchForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (!response.ok) {
            // conditional with negation operator to handle error
...
function displayGifs() {
    // declare function that displays gifs with delete button
    gifContainer.innerHTML = ''; // clear all gifs
    gifs.forEach((gif, index) => {
        // iterate over each gif in the gifs array (where we store gifs every time search button is pressed)
...
function deleteGif(index) {
    // delete gif from an array of gifs and display gifs without the deleted one
    gifs.splice(index, 1);
    displayGifs();
}

deleteAllButton.addEventListener('click', () => {
    // clear the gifs array and display the initial page (without gifs)
    gifs = [];
    displayGifs();
});
